;https://www.autohotkey.com/boards/viewtopic.php?t=53
...
class _String {
  ...
  getSubStringOrError(text,subString){
    if (this.contains(text, subString) == false){
      throw "Substring Not Found"
    }

    return InStr(text, subString)
  }
...
output:  aaazzzccc
*/
overwrite(overwrite, into, pos=1)
{
...
output:  aaaccc
*/
delete(string, start=1, length=1)
{
...
output: +++++aaa-^
*/
pad(string, left="0", right="", LCount=1, RCount=1)
{
   if (LCount>0)
   {
      if (LCount>1)
...
outputs: abc
*/
word(string, wordNum=1, Delim=" ", temp="¢")
{
...
*/
subString(string,search1,direction:="R",match:=1,search2:="",CaseSensitive:="")
{ ;Credit @ AfterLemon
...
*/
jumble(Text, Weight=1, Delim = "`n", Omit = "`r")
{
...
*/
insertLine(insert, into, line, delim="`n", exclude="`r")
{
...
output:  aaa|bbb|ccc
*/
deleteLine(string, line, delim="`n", exclude="`r")
{
...
output:  ddd
*/
readLine(string, line, delim="`n", exclude="`r")
{
   StringReplace, string, string, %delim%, %delim%, UseErrorLevel
   count:=ErrorLevel+1
...
---mmnnnoooppp
*/
lineWrap(string, column= 56, indentChar= "")
{
...
---red.
*/
wordWrap(string, column=56, indentChar="")
{
...
[2]
    ...
    [3] ==> 333
*/
printArr(array, depth=5, indentLevel="")
{
   for k,v in Array
   {
      list.= indentLevel "[" k "]"
      if (IsObject(v) && depth>1)
         list.="`n" this.printArr(v, depth-1, indentLevel . "    ")
      Else
         list.=" => " v
      list.="`n"
   }
...
output: 8
*/
countArr(array, depth=5, count=0)
{
   for k,v in Array
   {
      if (IsObject(v) && depth>1)
         count:= this.countArr(v, depth-1, count)
      count+=1
   }
   return count
}
...
;MsgBox % "This is an error message.`nThere was an error and you need to do something`rGet more info"
